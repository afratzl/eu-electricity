...
def generate_map(geodata, values_by_country, source, date_str, scale='fixed'):
    ...
    for i, (cc, val) in enumerate(sorted_countries):
        ...
        fig.text(fx + flag_w / 2, fy - flag_h / 2 - 0.007, cc,
                 ...
                 color='#333', zorder=12)
        # % to right of flag
        fig.text(fx + flag_w + 0.008, fy - flag_h * 0.15, value_str,
                 transform=fig.transFigure,
...
def load_summary_table_for_country(gc, country_code):
    ...
    # If ALL yesterday values are 0.0, country didn't report -- treat as no data
    yesterday_vals = [v['yesterday'] for v in result.values() if v['yesterday'] is not None]
    if yesterday_vals and all(v == 0.0 for v in yesterday_vals):
        for k in result:
            result[k]['yesterday'] = None

    # Same for last_week
    lastweek_vals = [v['last_week'] for v in result.values() if v['last_week'] is not None]
    if lastweek_vals and all(v == 0.0 for v in lastweek_vals):
        for k in result:
            result[k]['last_week'] = None
...
def main():
    ...
    if args.period in ('yesterday', 'last_week') and not args.all_months:
        ...
        for i, country_code in enumerate(countries_to_load):
            print(f"  Loading {country_code} ({i+1}/{len(countries_to_load)})...")
            try:
    ...
    for source in sources:
        ...
        if args.period == 'yesterday':
            ...
            # Add EU aggregate to legend
            values['EU'] = summary_table_data.get('EU', {}).get(source, {}).get('yesterday')
            ...
            for country_code in ENTSOE_COUNTRIES:
                try:
                    ...
                    values[country_code] = None
            # Add EU aggregate to legend
            values['EU'] = summary_table_data.get('EU', {}).get(source, {}).get('last_week')
            fig       = generate_map(geodata, values, source, date_str, scale=args.scale)
