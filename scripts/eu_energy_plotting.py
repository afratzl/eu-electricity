...
# === HELPER FUNCTIONS (FROM BEFORE VERSION) ===
...
def add_flag_and_labels(fig, country_code, main_title, subtitle):
    ...
    if os.path.exists(flag_path):
        try:
            ...
            ax_flag = fig.add_axes([0.03, 0.89, 0.12, 0.08])
    ...
    fig.text(0.03, 0.883, country_display,
             fontsize=20, fontweight='normal',
             ha='left', va='top',
             color='#333')
    
    # Add main title
    fig.text(0.55, 0.935, main_title,
...
def create_all_charts(all_data, country_code='EU'):
    ...
    MERGED: Includes Google Drive uploads from AFTER version
    
    Returns: (plot_links dict, drive_service object)
    """
    if not all_data:
    ...
    # Initialize Google Drive service for uploading plots (ADDED FROM AFTER)
    drive_service = initialize_drive_service()
    ...
    for source_name in sources_to_plot:
        ...
        ax1.legend(loc='upper left', bbox_to_anchor=(0.165, 0.1575), 
        ...
        ax2.legend(loc='upper left', bbox_to_anchor=(0.165, 0.1575),
    ...
    if available_sources and 'Total Generation' in all_data:
        ...
        for period in periods:
            ...
            ax1.legend(reordered_handles, reordered_labels, loc='upper left', bbox_to_anchor=(0.2, 0.1575),
            ...
            ax2.legend(reordered_handles, reordered_labels, loc='upper left', bbox_to_anchor=(0.2, 0.1575),
    ...
    if 'All Renewables' in all_data and 'All Non-Renewables' in all_data and 'Total Generation' in all_data:
        ...
        for period in periods:
            ...
            ax1.tick_params(axis='both', labelsize=22, length=8, pad=8)
            ax1.grid(True, linestyle='--', alpha=0.7)
            ax1.legend(loc='upper left', bbox_to_anchor=(0.30, 0.1575),
            ...
            ax2.tick_params(axis='both', labelsize=22, length=8, pad=8)
            ax2.grid(True, linestyle='--', alpha=0.7)
            ax2.legend(loc='upper left', bbox_to_anchor=(0.30, 0.1575),
    ...
    if all_sources_for_annual and 'Total Generation' in annual_totals:
        ...
        if True:  # Always true since we force all sources
            ...
            ax1.legend(reordered_handles, reordered_labels, loc='upper left', bbox_to_anchor=(0.19, 0.1575),
                       bbox_transform=fig1.transFigure, ncol=4,
                       fontsize=18, frameon=False)
        ...
        if True:  # Always true since we force all sources
            ...
            ax2.legend(reordered_handles, reordered_labels, loc='upper left', bbox_to_anchor=(0.19, 0.1575),
                       bbox_transform=fig2.transFigure, ncol=4,
                       fontsize=18, frameon=False)
    ...
    if available_totals and 'Total Generation' in annual_totals:
        ...
        if lines_plotted > 0:
            ...
            ax1.tick_params(axis='both', labelsize=22, length=8, pad=8)
            ax1.grid(True, linestyle='--', alpha=0.7)
            ax1.legend(loc='upper left', bbox_to_anchor=(0.30, 0.255),
                       bbox_transform=fig1.transFigure, ncol=2,
                       fontsize=18, frameon=False)
    ...
    # Return plot links and drive service (MODIFIED FROM AFTER)
...
def main():
    """
    Main function
    MERGED: Includes Drive links JSON saving from AFTER version
    """
    import argparse
    ...
    print("=" * 60)
    print(f"ENERGY PLOTTER - MERGED VERSION")
    print("=" * 60)
    ...
    print("  ✓ Google Drive uploads (ADDED)")
    print("  ✓ Correct Google Sheets updates (PRESERVED)")
    print("=" * 60)
    ...
    # Store all plot links by country (ADDED FROM AFTER)
    all_plot_links = {}
    ...
    for country_code in countries_to_process:
        ...
        # Create charts and get plot links (MODIFIED FROM AFTER)
