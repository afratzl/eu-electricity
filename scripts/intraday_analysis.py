...
def collect_all_data(api_key):
    ...
    for source in ATOMIC_SOURCES:
        ...
        for period_name, (start_date, end_date) in periods.items():
            ...
            # Always extend the query by 1 hour -- this correctly captures
            # each period's 23:00-24:00 slot for its last day (ENTSO-E's
            # exclusive-end boundary would otherwise clip it). Previously
            # this was only done for today/yesterday, causing week_ago/
            # year_ago/two_years_ago to silently average their 23:00-24:00
            # slot across only 6 days instead of 7 for that one hour.
            extra_hour = True
            ...
            # Trim any spillover past the period's own intended end_date --
            # extending the query by 1 hour above also pulls in the first
            # timestamp of the NEXT day (an accidental 8th day), which this
            # removes while keeping the correctly-captured last hour.
            if not country_df.empty:
                country_df = country_df[country_df.index < pd.Timestamp(end_date, tz='Europe/Brussels')]
    ...
    for source in AGGREGATE_SOURCES:
        ...
        for period_name, (start_date, end_date) in periods.items():
            mark_extrap = (period_name in ['today', 'yesterday'])
            # Same fix as atomic_sources above -- always capture the last
            # hour, then trim any accidental 8th-day spillover.
            extra_hour = True
            ...
            if not eu_total.empty:
                eu_total = eu_total[eu_total.index < pd.Timestamp(end_date, tz='Europe/Brussels')]
    ...
    for period_name, (start_date, end_date) in periods.items():
        mark_extrap = (period_name in ['today', 'yesterday'])
        # Same fix as above -- always capture the last hour, then trim
        # any accidental 8th-day spillover.
        extra_hour = True
...
def extract_country_from_raw_data(raw_data_matrix, country_code):
    ...
    for source in ATOMIC_SOURCES:
        ...
        for period_name, country_df in raw_data_matrix['atomic_sources'][source].items():
            ...
            if country_code == 'EU':
                # Keep all EU countries (individual, not yet summed) --
                # threshold detection needs individual country data.
                # Summing happens AFTER correction in apply_corrections_for_period.
                # IMPORTANT: country_df actually has all 36 tracked countries
                # at this point (EU + non-EU like Norway/Switzerland get
                # concatenated onto the same columns during collection), so
                # explicitly filter to real EU members here -- otherwise
                # every downstream sum (today/yesterday corrections,
                # week/year-ago stats, total_generation) silently inflates
                # with non-EU countries' generation, hydro especially given
                # Norway alone is ~90%+ hydro.
                eu_cols = country_df.columns.intersection(EU_COUNTRIES)
                extracted_data['atomic_sources'][source][period_name] = country_df[eu_cols].copy()
            else:
    ...
    for period_name, country_df in raw_data_matrix['total_generation'].items():
        ...
        if country_code == 'EU':
            # Keep all EU countries (individual, not yet summed) --
            # threshold detection needs individual country data.
            # Same fix as atomic_sources above: filter to real EU members,
            # since country_df actually has all 36 tracked countries here.
            eu_cols = country_df.columns.intersection(EU_COUNTRIES)
            extracted_data['total_generation'][period_name] = country_df[eu_cols].copy()
        else:
            # Extract single country - keep as DataFrame with country column
            if country_code in country_df.columns:
                extracted_df = country_df[[country_code]].copy()
...
def apply_projections_and_corrections(data_matrix):
    """
    Phase 2: Apply 1% threshold and correct aggregates/total_gen using atomic sources
...
def convert_corrected_data_to_plot_format(source_type, corrected_data, country_code='EU'):
    """
    ...
    for period_name, period_data in corrected_data.items():
        ...
        for timestamp in sorted(source_data.keys()):
            # source_data[timestamp] is always a dict of country -> value,
            # for every source type and every period -- this matches what
            # the original code did unconditionally before any of these
            # fixes. Sum across countries for this source, filtered to the
            ...
            # combined structure -- massively inflating hydro specifically,
            # since Norway alone is ~90%+ hydro.
            timestamp_data = source_data[timestamp]
            if country_code == 'EU':
                relevant_values = [
                    v for k, v in timestamp_data.items()
                    if (k in EU_COUNTRIES or k == 'EU') and v is not None
                ]
            else:
                # Aggregate sources (all-renewables/all-non-renewables)
                # always store their value under the literal key 'EU',
                # regardless of which country is actually being processed
                # (e.g. even Germany's own data ends up as {'EU': value}
                # here) -- fall back to it when the real country_code
                # isn't present, since normal atomic per-country dicts use
                # the actual country code as the key instead.
                v = timestamp_data.get(country_code, timestamp_data.get('EU'))
                relevant_values = [v] if v is not None else []
            energy_prod = sum(relevant_values)
            
            # total_gen_data[timestamp] is a plain scalar -- unchanged from
            # the original code's treatment, no filtering needed here.
...
def plot_analysis(stats_data, source_type, output_file_base, country_code='EU'):
    ...
    ax1.set_ylabel('Electrical Power (%)', fontsize=24, fontweight='bold', labelpad=10)
    ...
    plt.savefig(output_file_percentage, dpi=150)
    ...
    ax2.set_ylabel('Electrical Power (GW)', fontsize=24, fontweight='bold', labelpad=10)
    ...
    if len(yticks) > 1:
        ...
        if unit_label is not None:
            ...
            # Reapply formatting
            ax2.set_xlabel('Time of Day (Brussels)', fontsize=24, fontweight='bold', labelpad=10)
            ax2.set_ylabel(unit_label, fontsize=24, fontweight='bold', labelpad=10)
            ax2.tick_params(axis='both', labelsize=22, length=8, pad=8)
    ...
    plt.savefig(output_file_absolute, dpi=150)
...
def generate_plot_for_source(source_type, corrected_data, output_file_base, fetch_time=None, country_code='EU'):
    ...
    plot_data = convert_corrected_data_to_plot_format(source_type, corrected_data, country_code=country_code)
...
def calculate_period_totals(period_data, period_name, country_code='EU'):
    ...
    # Get total generation -- plain scalar sum, unchanged from the original
    total_gen_data = period_data.get('total_generation', {})
    # Convert MW to GWh: MW * 0.25 hours (15-min intervals) / 1000
    total_gen_gwh = sum(total_gen_data.values()) * 0.25 / 1000
    
    def eu_country_sum(countries):
        # countries is always a dict of country -> value here, matching
        # what the original code did unconditionally. Sum filtered to the
        # requested selection -- without this filter, 'EU' silently summed
        # every country present, including non-EU countries (Norway,
        # Switzerland, etc.) stored in the same combined structure --
        # massively inflating hydro specifically, since Norway alone is
        # ~90%+ hydro. This is what fed the Summary Table's Yesterday/Last
        # Week columns with an inflated value.
        ...
        if country_code == 'EU':
            return sum(v for k, v in countries.items() if (k in EU_COUNTRIES or k == 'EU') and v is not None)
        else:
            # Same fallback as convert_corrected_data_to_plot_format --
            # aggregate sources always store their value under the literal
            # key 'EU', regardless of which country is actually being
            # processed, so fall back to it when the real country_code
            # isn't present as a key.
            v = countries.get(country_code, countries.get('EU'))
    ...
    for source in ATOMIC_SOURCES:
        ...
        for timestamp, countries in source_data.items():
            source_total_mw += eu_country_sum(countries)
    ...
    for agg_source in AGGREGATE_SOURCES:
        ...
        # Sum all timestamps (EU-filtered)
        agg_total_mw = 0
        for timestamp, countries in agg_data.items():
            agg_total_mw += eu_country_sum(countries)
...
def generate_yesterday_plots(corrected_data, country_code='EU'):
    ...
    fig2.savefig(absolute_file, dpi=150, facecolor='white')
    plt.close(fig2)
